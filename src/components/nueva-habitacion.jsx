...
function NuevaHabitacion() {
  ...
  const [showAlert, setShowAlert] = useState(false); // Nuevo estado para controlar la visualización temporal del mensaje
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowAlert(true); // Mostrar el mensaje
      setTimeout(() => {
        setShowAlert(false); // Ocultar el mensaje después de 2 segundos
        window.location.href = "/habitaciones"; // Redirigir utilizando window.location.href
      }, 2000);
    } catch (error) {
  ...
  return (
    <div
    className=" mb-0 bg-teal-50 "
    >
    <Navbar />
    <div className=" mb-0 bg-teal-50 flex flex-col justify-center items-center">
      
      <h1
      className="text-teal-500 text-4xl font-semibold mt-4 mb-4"
      >Agregar Nueva Habitación</h1>
      {showAlert && <div className="alert text-lime-500 text-4xl font-semibold ">{alertMessage}</div>}
      <form onSubmit={handleSubmit} className="w-full max-w-md px-4">
        <div className="mb-4">
          <label className="block text-teal-500 text-xl font-semibold  ">Número de Habitación:</label>
          <input
            type="text"
            name="roomNumber"
        ...
        <div className="mb-4">
          <label className="block text-teal-500 text-xl font-semibold">Estado:</label>
          <select
            name="roomState"
        ...
        <div className="mb-4">
          <label className="block text-teal-500 text-xl font-semibold">Plazas:</label>
          <input
            type="text"
            name="roomBeds"
        ...
        <button type="submit"            className="text-white font-semibold bg-teal-500 hover:bg-gray-900 text-base focus:outline-none 
